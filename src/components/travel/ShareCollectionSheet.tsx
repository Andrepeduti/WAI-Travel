...
// Mantido para compatibilidade com outros sheets (CreateCollection, AddVideo...)
// que usam essa lista para mostrar avatares de pré-visualização localmente.
export const mockPeople: SharePerson[] = [
  { id: '2', name: 'Ana Silva', email: 'ana@email.com', initials: 'AS', color: '#10B981', photo: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop&crop=face' },
  { id: '3', name: 'Bruno Costa', email: 'bruno@email.com', initials: 'BC', color: '#F59E0B', photo: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&fit=crop&crop=face' },
  { id: '4', name: 'Carla Dias', email: 'carla@email.com', initials: 'CD', color: '#8B5CF6', photo: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=100&h=100&fit=crop&crop=face' },
  { id: '5', name: 'Diego Martins', email: 'diego@email.com', initials: 'DM', color: '#EF4444', photo: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=100&h=100&fit=crop&crop=face' },
  { id: '6', name: 'Fernanda Lima', email: 'fernanda@email.com', initials: 'FL', color: '#EC4899', photo: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=100&h=100&fit=crop&crop=face' },
  { id: '7', name: 'Gabriel Souza', email: 'gabriel@email.com', initials: 'GS', color: '#14B8A6', photo: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop&crop=face' },
  { id: '8', name: 'Helena Rocha', email: 'helena@email.com', initials: 'HR', color: '#F97316', photo: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?w=100&h=100&fit=crop&crop=face' },
  { id: '9', name: 'Igor Pereira', email: 'igor@email.com', initials: 'IP', color: '#3B82F6', photo: 'https://images.unsplash.com/photo-1463453091185-61582044d556?w=100&h=100&fit=crop&crop=face' },
  { id: '10', name: 'Juliana Alves', email: 'juliana@email.com', initials: 'JA', color: '#10B981', photo: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&h=100&fit=crop&crop=face' },
];
...
export function ShareCollectionSheet({
  ...
  // Carrega owner + contatos mútuos (eu sigo e me seguem de volta)
  useEffect(() => {
    if (!isOpen) return;
    let cancelled = false;
    (async () => {
      try {
        ...
        if (!user) {
          if (!cancelled) { setOwnerId(null); setMutualIds([]); }
          return;
        }
    ...
    const handle = setTimeout(async () => {
      try {
        ...
        let req = supabase
          ...
          .limit(10);
        if (ownerId) req = req.neq('user_id', ownerId);
  ...
  }, [query, isOpen, ownerId, selectedUser, mutualIds]);
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      if (!selectedIds.includes(selectedUser.user_id)) {
        onToggle(selectedUser.user_id);
      }
  ...
  const handleCopyLink = async () => {
    ...
    setCopying(true);
    try {
      const url = `${window.location.origin}/colecao/convite/${crypto.randomUUID()}`;
    ...
    <div className="fixed inset-0 z-[120] flex items-end justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="px-5 pt-2 pb-4">
          <h3 className="text-[18px] font-bold text-foreground">Compartilhar coleção</h3>
          {collectionName && <p className="text-[13px] text-muted-foreground mt-0.5">{collectionName}</p>}
        </div>
